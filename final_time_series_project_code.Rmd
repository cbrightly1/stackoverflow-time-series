...
```{r}
# Vectorized method that is not prone to error
tmp <- matrix(rep(0, length(python.forecast.time)^2), ncol = length(python.forecast.time))
diag(tmp) <- rep(coef(python.detrend.lm), length.out = ncol(tmp))
python.fcsts <- rbind(python.fcsts, tmp)
rm(tmp)
```
...
> The forecasted values indicated that the positive trend will continue to 2022. Python’s predicted number of questions is forecasted to grow from 20,058 in December 2020 to 24,001 in December 2022, a 19.7% growth rate.
...
```{r}
tmp <- matrix(rep(0, length(r.forecast.time)^2), ncol = length(r.forecast.time))
diag(tmp) <- rep(coef(r.detrend.lm), length.out = ncol(tmp))
r.fcsts <- rbind(r.fcsts, tmp)
rm(tmp)
```
...
> The forecasted values indicated that the positive trend will continue to 2022. R’s predicted number of questions is forecasted to grow from 4,150 in December 2020 to 5,380 in December 2022, a 29.7% growth rate.
